...
def calcular_medidas_descritivas(dados_array_final):
    """
    ...
    """
    if dados_array_final is None or len(dados_array_final) == 0:
        return None

    # Medidas de Tendência Central
    media = np.mean(dados_array_final)
    mediana = np.median(dados_array_final)

    # Medidas de Posição (Quartis e IQR)
    Q1 = np.percentile(dados_array_final, 25)
    Q3 = np.percentile(dados_array_final, 75)
    IQR = Q3 - Q1
    ...
    # Valores Extremos
    min_valor = np.min(dados_array_final)
    max_valor = np.max(dados_array_final)
...
def gerar_painel_boxplot(dados_array_final, medidas, titulo_boxplot='Boxplot da Distribuição de Dados', caminho_salvar=None):
    """
    ...
    axes[0].set_ylabel(dados_array_final.name if hasattr(dados_array_final, 'name') else 'Valores')
